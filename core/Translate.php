...
class Translate
{
    ...
    public static function autoInit($content)
    {
        ...
        if (self::$_autoTranslate) {
            ...
            if (Settings::getInstance()->getVar('showRuntime')) {
    ...
    public static function getLocalizedLinks($content)
    {
        ...
        $content = preg_replace('#<a([^>]*)href=("|\')([^>]+\.(?:jpg|png|gif|pdf|jpeg|zip|rar|tar|ico|mp3))#Usi',
            '<a$1href_sli_file=$2$3', $content);

        //Локализируем
        $allow   = self::getAllowPregString();
        $host    = preg_quote($_SERVER['HTTP_HOST']);
        $content = preg_replace('#<(a|base)(?! %)([^>]*)href=("|\')((/)(?!' . self::$language . '/)|(https?://' . $host . ')(?!/' . self::$language . '/))(' . ($allow ? $allow : '[^>]*') . ')(?!\\\)\\3(.*)>#Usi',
            '<$1$2href=$3$6/' . self::$language . '$5$7$3$8>', $content);
        $content = preg_replace('#<form([^>]*)action=("|\')((/)(?!' . self::$language . '/)|(https?://' . $host . ')(?!/' . self::$language . '/))(' . ($allow ? $allow : '[^>]*') . ')(?!\\\)\\2(.*)>#Usi',
            '<form$1action=$2$5/' . self::$language . '$4$6$2$7>', $content);
        $content = preg_replace('#(?:document\.)?location\.href\s*=\s*("|\')((/)(?!' . self::$language . '/)|(https?://' . $host . ')(?!/' . self::$language . '/))(' . ($allow ? $allow : '.*') . ')(?!\\\)\\1#Ui',
            'location.href=$1$4/' . self::$language . '$3$5$1', $content);
    ...
    {
        if (self::getCurrentLanguage() && is_string($url) && ($url[0] == '/' || strpos($url,
                    'http://') !== false || strpos($url, 'https://') !== false)
        ) {
            $allow = self::getAllowPregString();
            $url   = preg_replace('#^((?:(/)(?!' . self::$language . '/))|(?:(https?://' . preg_quote($_SERVER['HTTP_HOST']) . ')(?!/' . self::$language . '/)))(' . ($allow ? $allow : '.*') . ')$#Ui',
                '$3/' . self::$language . '$2$4', $url);
        }
    ...
    {
        $result = false;
        $rules  = Settings::getInstance()->getVar('igroreUrls', true);
    ...
    {
        $result = false;
        $rules  = Settings::getInstance()->getVar('ignorePageVsContent', true);
    ...
    public static function getLocalizedContent($content)
    {
        ...
        if (Settings::getInstance()->getVar('cacheStatus')) {
            ...
            $cacheContent = Cache::getInstance($cacheId)->get();
            if ($cacheContent) {
        ...
        if (!Settings::getInstance()->getVar('cacheStatus') || !$cacheContent) {
                ...
                foreach ($originalData['original'] AS $k => $v) {
                    ...
                    if ($isHaveTranslate) {
                        ...
                        if (self::$_tData[$search]['original'] != $clean) {
                            ...
                            preg_match_all('#(?:(?:&\#?[A-z0-9]{1,7};)|[\d,.|!?/\#*+=^~_&^%$@:©×()\[\]{}"\\\;])+#u',
                                $clean, $symbols);
                            preg_match_all('#(?:(?:&\#?[A-z0-9]{1,7};)|[\d,.|!?/\#*+=^~_&^%$@:©×()\[\]{}"\\\;])+#u',
                                $isHaveTranslate, $tSymbols);
                            ...
                            if (!empty($symbols)) {
                                ...
                                foreach ($symbols as $symbolKey => $symbol) {
                                    ...
                                    if (!isset($tSymbols[$symbolKey]) || $tSymbols[$symbolKey] == '&#039;') {
                                        continue;
                                    }
                                    $sPos = strpos($isHaveTranslate, $tSymbols[$symbolKey], $sPos);

                                    if ($sPos !== false) {
                                        $isHaveTranslate = substr_replace($isHaveTranslate, $symbol, $sPos,
            ...
            $originalVars  = Vars::getOriginalContent();
            $translateVars = Vars::getLanguageContent(self::$language);
            if (!empty($originalVars) && is_array($originalVars)) {
                foreach ($originalVars as $key => $var) {
                    if (strpos($content, $var) === false || empty($translateVars[$key])) {
                        continue;
                    }
            ...
            if (Settings::getInstance()->getVar('cacheStatus') && !empty($content)) {
                Cache::getInstance($cacheId)->set($content, Settings::getInstance()->getVar('cacheTime'));
            }
        }
    ...
    public static function getIgnoreTags()
    {
        ...
        $ignoreStr  = Settings::getInstance()->getVar('ignoreTags');

        if ($ignoreStr) {
            $ignoreTags = explode(',', $ignoreStr);
            foreach ($ignoreTags as &$value) {
                $value = trim($value);
            }
        }
    ...
    {
        $ignoreTags = ['title', 'alt', 'placeholder', 'content'];
        $ignoreStr  = Settings::getInstance()->getVar('visibleAttr');
     ...
     * @return array
     */
    public static function createOriginalKeys($searchKeys = [])
    {
        $keys = [];
    ...
    public static function getCurrentLanguage()
    {
        if (is_null(self::$language)) {
            ...
            //парсим урл
            preg_match('!^/(' . implode('|', self::getLanguages($onlyActive)) . ')(/|\?|\Z)!', $_SERVER['REQUEST_URI'],
                $match);
