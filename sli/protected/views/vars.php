...
<?php SLIAdmin::renderPartial('parts/_create_form', array('languages' => $languages, 'search' => $search, 'language' => $language,));?>
	...
	$(function(){
	    SLItranslateApi.itemsBlock.on('change', 'textarea', function(){
            ...
            if (currId) {	       
    	        ...
    	        $.post('?action=saveVarTranslate', {language: $(this).attr('lang'), idContent: currId, content: $(this).val()}, function(){
    	        	site.setVisualLoad();
    	        });
            }            
	    });
	    ...
	    $('#selectedDelete').click(function(){
	        ...
	        if (countItems && confirm('Удалить '+countItems+' выбранных элементов?')) {
	        	...
	        	site.setVisualLoad(true);
	            $.post('?action=deleteVar', {idContent: deleteIds}, function(data){
	                site.setVisualLoad();
	                if (data) {
